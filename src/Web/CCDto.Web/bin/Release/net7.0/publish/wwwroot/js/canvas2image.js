	...
	var saveAsImage = function (canvas, width, height, type, filename) {
	    if ($support.canvas && $support.dataURL) {
	        ...
	        if (/bmp/.test(type)) {
	            ...
	            saveFile1(canvas, makeURI(strData, downloadMime), filename);
	        } else {
	            var strData = getDataURL(canvas, type, width, height);
	            saveFile1(canvas, strData.replace(type, downloadMime), filename);
	        }

	    }
	}
	var saveFile1 = function (canvas, data, filename) {
