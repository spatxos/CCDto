...
+ function ($) {
    ...
    //如果参数过多，建议通过 config 对象进行配置，而不是传入多个参数。
    $.areamodal = function (text, title, onOK, onCancel, textarea) {
        var config;
        if (typeof text === 'object') {
            config = text;
        } else {
            if (typeof title === 'function') {
                textarea = arguments[3];
                onCancel = arguments[2];
                onOK = arguments[1];
                title = undefined;
            }
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">' + (config.text || '') + '</p><textarea class="weui-textarea weui-prompt-textarea" id="weui-prompt-textarea" value="' + (config.textarea || '') + '" rows="3"/>',
            title: config.title,
            autoClose: false,
            buttons: [
            {
            ...
            },
            {
                ...
                className: "primary",
                onClick: function () {
                    var textarea = $("#weui-prompt-textarea").val();
                    if (!config.empty && (textarea === "" || textarea === null)) {
                        modal.find('.weui-prompt-textarea').focus()[0].select();
                        return false;
                    }
                    $.closeModal();
                    config.onOK && config.onOK.call(modal, textarea);
                }
            }]
        }, function () {
            this.find('.weui-prompt-textarea').focus()[0].select();
        });
    ...
    //如果参数过多，建议通过 config 对象进行配置，而不是传入多个参数。
    $.selectmodal = function (text, title, onOK, onCancel, select) {
        var config;
        if (typeof text === 'object') {
            config = text;
        } else {
        ...
        var selectoption = "";
        for (var i = 1; i < 25; i++) {
            selectoption += '<option value=' + i + ' ' + (config.select == i ? 'selected=selected' : '') + '>' + i + '小时</option>';
        }

        var modal = $.modal({
            text: '<p class="weui-prompt-text">' + (config.text || '') + '</p><select class="weui-select weui-prompt-select" id="weui-prompt-select">' +
                selectoption + '</select>',
            title: config.title,
            autoClose: false,
            buttons: [
            ...
            },
            {
                ...
                className: "primary",
                onClick: function () {
                    var select = $("#weui-prompt-select").val();
                    if (!config.empty && (select === "" || select === null)) {
                        modal.find('.weui-prompt-select').focus()[0].select();
                        return false;
                    }
                    $.closeModal();
                    config.onOK && config.onOK.call(modal, select);
                }
            }]
        }, function () {
            //this.find('.weui-prompt-textarea').focus()[0].select();
